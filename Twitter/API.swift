...
class API {
    ...
    private func login(completion: (account: ACAccount?) -> ())
    {
        ...
        accountStore.requestAccessToAccountsWithType(accountType, options: nil, completion: {(granted, error) in
            ...
            if granted {
                if let account = accountStore.accountTypeWithAccountTypeIdentifier(accountType).first as?
                    ACAccount {
    ...
    {
        let request = SLRequest(forServiceType: SLServiceTypeTwitter, requestMethod: .GET, URL: NSURL(string: "https://api.twitter.com/1.1/account/verityf-credentials.json"), parameters: nil)
...
//update timeline function
    private func updateTimeline(completion: (tweets: [Tweet]? -> ())) {
        ...
        request.performRequestWithHandler {(data, response, error) in
            ...
            switch response.statusCode {
            case 200...299:
                JSONParser.tweetJSONFrom(data, completion: {(success, tweets) in dispatch_asynch(dispatch_get_main_queue(), {completion(tweets: tweet))})
            })
            case 400...499:
                print("Client error:  \(response.statusCode)")
                completion(tweets: nil)
